import * as React from "react";
import Table from "@mui/material/Table";
import TableBody from "@mui/material/TableBody";
import TableCell from "@mui/material/TableCell";
import TableContainer from "@mui/material/TableContainer";
import TableHead from "@mui/material/TableHead";
import TableRow from "@mui/material/TableRow";
import Paper from "@mui/material/Paper";
import EditIcon from "@mui/icons-material/Edit";
import DeleteIcon from "@mui/icons-material/Delete";
import { IconButton, Stack, useTheme } from "@mui/material";

function createData(name, calories, fat, carbs, protein,carbss,carbsss) {
  return { name, calories, fat, carbs, protein,carbss,carbsss };
}

const rows = [
  createData("Frozen yoghurt", 159, 6.0, 24, 4.0,4,3),
  createData("Ice cream sandwich", 237, 9.0, 37, 4.3,4,5),
  createData("Eclair", 262, 16.0, 24, 6.0,5,6),
  createData("Cupcake", 305, 3.7, 67, 4.3,6,6),
  createData("Gingerbread", 356, 16.0, 49, 3.9,6,6),
];

export default function Table_Orders() {
  const theme = useTheme();

  const headerBg =
    theme.palette.mode === "light" ? "#d628ab" : "#1e1e1e"; 
  // اختر لون النص حسب الخلفية
  const headerText =
    theme.palette.mode === "light"
      ? "#ffffff" 
      : "#f990e2"; 

  return (
    <TableContainer component={Paper}>
      <Table sx={{ minWidth: 650 }} aria-label="simple table">
        <TableHead>
          <TableRow sx={{ backgroundColor: headerBg, color: headerText }}>
            <TableCell sx={{ color: headerText }}>Order_ID</TableCell>
            <TableCell align="right" sx={{ color: headerText }}>
              Customer
            </TableCell>
            <TableCell align="right" sx={{ color: headerText }}>
              Products
            </TableCell>
            <TableCell align="right" sx={{ color: headerText }}>
              Price
            </TableCell>
            <TableCell align="right" sx={{ color: headerText }}>
              Date
            </TableCell>
            <TableCell align="right" sx={{ color: headerText }}>
              Status
            </TableCell>
            <TableCell align="right" sx={{ color: headerText }}>
              Action
            </TableCell>
          </TableRow>
        </TableHead>

        <TableBody>
          {rows.map((row) => (
            <TableRow
              key={row.name}
              sx={{ "&:last-child td, &:last-child th": { border: 0 } }}
            >
              <TableCell component="th" scope="row">
                {row.name}
              </TableCell>
              <TableCell align="right">{row.calories}</TableCell>
              <TableCell align="right">{row.fat}</TableCell>
              <TableCell align="right">{row.carbs}</TableCell>
                            <TableCell align="right">{row.carbsss}</TableCell>

              <TableCell align="right">{row.carbss}</TableCell>

              <TableCell align="right">
                <Stack direction="row" spacing={1} justifyContent="flex-end">
                  <IconButton color="primary" size="small">
                    <EditIcon />
                  </IconButton>
                  <IconButton color="error" size="small">
                    <DeleteIcon />
                  </IconButton>
                </Stack>
              </TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
    </TableContainer>
  );
}
